        ...
        <Animated.View entering={FadeIn.duration(280)} style={styles.paper}>
          ...
          <View style={styles.summaryRow}>
            ...
            <View style={styles.summaryCell}>
              ...
              <Text style={styles.summaryLabel}>
                {byRoom.size === 1 ? 'Room affected' : 'Rooms affected'}
              </Text>
            </View>
            <View style={styles.summaryCell}>
              <Text style={styles.summaryValue}>{rooms.length}</Text>
              <Text style={styles.summaryLabel}>Rooms total</Text>
            </View>
          </View>
          ...
          {cleanInspection && (
            <View style={styles.cleanBanner}>
              ...
              <Text style={styles.cleanBannerText}>
                All rooms reviewed. No defects logged — ready for handover.
              </Text>
            </View>
          )}
...
function buildReportText({
  ...
  lines.push('');
  lines.push(`${config.bhk} ${config.type.replace('-', ' ')} · ${rooms.length} rooms · ${defects.length} defect${defects.length === 1 ? '' : 's'}`);
  if (defects.length > 0) {
    lines.push(
  ...
  if (defects.length === 0) {
    lines.push('All rooms reviewed. No defects logged.');
    lines.push('');
